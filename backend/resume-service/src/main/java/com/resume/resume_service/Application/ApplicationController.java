...
public class ApplicationController {
    ...
    public ResponseEntity<?> applyToJob(
        ...
        // Note : Le matchScore sera null ici tant que l'IA n'a pas tourné ou qu'on ne l'a pas set
    ...
    // 2) Recruteur voit les candidatures pour une offre (avec calcul IA en direct)
    ...
    public List<ApplicationDto> listApplicationsForJob(
        ...
        // (Optionnel) Vérifier que l'offre appartient au recruteur connecté
        ...
        for (Application app : apps) {
            ...
            try {
                ...
                System.err.println("Erreur téléchargement CV pour app " + app.getId());
            }
        }
        ...
        if (!files.isEmpty()) {
            try {
                ...
                for (var r : results) {
                    ...
                    // OPTIONNEL : Sauvegarder le score en base pour qu'il soit visible plus tard
                    // Application appToUpdate = filenameToApp.get(r.filename());
                    // if(appToUpdate != null) {
                    //    appToUpdate.setMatchScore(r.score());
        ...
        // 3️⃣ Construire les DTO avec score
        ...
        for (Map.Entry<String, Application> entry : filenameToApp.entrySet()) {
            ...
            // On prend le score de l'IA s'il vient d'être calculé, sinon celui de la base
            Double score = scoreByFilename.get(partName);
            if (score == null) score = app.getMatchScore();

            out.add(ApplicationDto.fromEntity(app, score));
        }

        // 4️⃣ Trier par score décroissant (nulls en bas)
        out.sort((a, b) -> {
